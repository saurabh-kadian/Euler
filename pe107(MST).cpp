...
int main(){
    ...
    long long int graph[40][40] = {0,0,0,427,668,495,377,678,0,177,0,0,870,0,869,624,300,609,131,0,251,0,0,0,856,221,514,0,591,762,182,56,0,884,412,273,636,0,0,774
	...
	vector<vector<long long int> >newGraph;
	long long int total = 0;
	for(int i=0;i<40;++i){
        vector<long long int> thiser;
        for(int j=0;j<40;++j){
            thiser.push_back(graph[i][j]),total += graph[i][j];
        }
        newGraph.push_back(thiser);
	}
    set<long long int> passed;
    set<long long int> notPassed;
    for(int i=1;i<40;++i)
        notPassed.insert(i);
    passed.insert(0);
    long long int thisTotal = 0;
    while(notPassed.size() != 0){
        vector<pair<long long int,long long int> >nextPossible;
        for(set<long long int>::iterator it = passed.begin();it != passed.end();++it){
            for(long long int i=0;i<newGraph[*it].size();++i){
                if(newGraph[*it][i] != 0){
                    bool found = false;
                    for(set<long long int>::iterator itq = notPassed.begin();itq != notPassed.end();++itq)
                        if(*itq == i)
                            found = true;
                    if(found)
                        nextPossible.push_back(pair<long long int,long long int>(*it,i));
                }
            }
        }
        long long int minVal = 999999,minIndex = 41,minFrom = 41;
        for(long long int i=0;i<nextPossible.size();++i){
            if(newGraph[nextPossible[i].first][nextPossible[i].second] < minVal)
                minVal = newGraph[nextPossible[i].first][nextPossible[i].second],minIndex = nextPossible[i].second,
                            minFrom = nextPossible[i].first;
        }
        if(minIndex != 41 && minFrom != 41)
            thisTotal += newGraph[minFrom][minIndex];// For Edges ,cout << "Edge : " << minFrom << " to " << minIndex << endl;
        notPassed.erase(minIndex);
        passed.insert(minIndex);
    }
    cout << "ProjectEuler 107 (Prim's) :\n " << "\t\t\t\t Saving : " <<  total/2-thisTotal << endl
        << "\t\t\t\t Original Total : "<< " " << total/2 << endl
        << "\t\t\t\t New Total : " << " " << thisTotal << endl;
	return 0;
}
